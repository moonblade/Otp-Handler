...
def generate_token_OTP(mobile, request_header, auto = False):
    """
    ...
    while not valid_token:
        try:
            beforeReq = round(time.time() * 1000)
            data = {"mobile": mobile,
            ...
            if txnId.status_code == 200:
                ...
                if auto:
                    while OTP is None:
                        otpR = requests.get("https://otp-tracker.herokuapp.com?mobile=" + mobile)
                        otpData = otpR.json();
                        if int(otpData["lastTime"]) > beforeReq:
                            OTP = int(otpData["otp"])
                        time.sleep(1)
                else:
